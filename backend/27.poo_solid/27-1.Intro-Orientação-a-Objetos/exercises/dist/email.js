...
class MailList {
    ...
    getEmailsSentBy(mail) {
        return this.mailList.filter((email) => email.from === mail);
    }
    getByEmailsSentTo(mail) {
        return this.mailList.filter((email) => email.to === mail);
    }
    getEmailsBySubject(mail) {
        return this.mailList.filter((email) => email.subject.includes(mail));
    }
    addEmail(email) { this.mailList = [...this.mailList, email]; }
    removeEmail(email) { this.mailList = this.mailList.filter((mail) => email !== mail); }
}
// Para testar
...
personalMailList.getEmailsSentBy(myEmail).forEach((mail) => console.log(mail.content));
